...
impl SerdeEnumTag {
    ...
    pub fn from_attrs(attrs: &[Attribute]) -> Result<Self> {
        let conv = if let Some(tag) = serde_meta_name_value(attrs, "tag")? {
            let tag = meta_value_as_str(&tag)?;

            if let Some(content) = serde_meta_name_value(attrs, "content")? {
                let content = meta_value_as_str(&content)?;

                SerdeEnumTag::Adjacent { tag, content }
            } else {
                SerdeEnumTag::Internal(tag)
            }
        } else if has_serde_meta_word(attrs, "untagged")? {
